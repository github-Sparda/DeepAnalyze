...
class CollaborationManager:
    ...
    def _save_collaboration_data(self, session_id: str):
        """保存协作数据到文件"""
        try:
            collab_dir = self.workspace_base / session_id / "collaboration"
    ...
    def _save_share_links(self):
        """保存分享链接数据"""
        try:
            collab_dir = self.workspace_base / "shared"
            collab_dir.mkdir(parents=True, exist_ok=True)
    ...
    def _save_comments(self):
        """保存评论数据"""
        try:
            collab_dir = self.workspace_base / "comments"
            collab_dir.mkdir(parents=True, exist_ok=True)
...
# 便捷函数
def get_collaboration_manager(workspace_base: str = "workspace") -> CollaborationManager:
    """获取协作管理器实例"""
    return CollaborationManager(workspace_base)
