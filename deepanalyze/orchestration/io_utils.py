...
def init_workspace(workspace_dir: str | Path) -> dict[str, Path]:
...
def _load_manifest(workspace_dir: str | Path) -> list[dict[str, Any]]:
...
def record_artifact(
    ...
    write_json(Path(workspace_dir) / "manifest.json", manifest)
...
def record_node_log(
    ...
    log_dir = ensure_dir(Path(workspace_dir) / WORKSPACE_DIRS["log"] / "nodes")
...
def record_run_summary(workspace_dir: str | Path, summary: dict[str, Any]) -> Path:
    meta_dir = ensure_dir(Path(workspace_dir) / WORKSPACE_DIRS["meta"])
...
def write_run_metadata(workspace_dir: str | Path, metadata: dict[str, Any]) -> Path:
    meta_dir = ensure_dir(Path(workspace_dir) / WORKSPACE_DIRS["meta"])
