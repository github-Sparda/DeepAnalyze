    ...
    def __post_init__(self):
        if self.tmpfs_mounts is None:
            self.tmpfs_mounts = {
                ...
                "/workspace": "rw,noexec,nosuid,size=200m"
            }
...
class ContainerSession:
    ...
    def reset_state(self):
        ...
        try:
            # 清理工作目录
            self.container.exec_run("rm -rf /workspace/*")
            # 重置访问计数
            self.executions = 0
...
class ContainerPool:
    ...
    def _create_container(self):
        """创建单个容器"""
        container_name = f"deepanalyze-worker-{uuid.uuid4().hex[:8]}"
    ...
    def _install_dependencies(self, container):
        ...
        if requirements:
            cmd = f"pip install --no-cache-dir {' '.join(requirements)}"
            try:
    ...
    def execute_in_session(self, user_id: str, code: str, 
        ...
        try:
            ...
            exec_cmd = f"echo '{code}' > /workspace/{code_filename} && python /workspace/{code_filename}"
