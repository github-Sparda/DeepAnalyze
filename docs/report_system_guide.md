...
当计划与执行不一致时，必须增加“偏差说明”，并明确后续分析以实际执行结果为主。

### 术语统一（避免内部术语直出）
- 建议使用“判定规则/判定状态”，不直接使用“门槛类型/门槛状态”。
- 建议使用“计划假设/实际执行假设”，不使用“计划口径/执行口径”。
- 建议使用“效应量证据”，不使用“效应证据”。
- 建议使用“判定阈值”，不使用“阈值口径”。

### 方法步骤展示
- 优先展示可执行步骤，不展示“预期产物/成功判据/后续行动”等计划元信息。
- 在可行时，为步骤补充输入/输出注释（例如输出文件名与执行状态）。
- 当缺少结构化步骤时，必须明确标注“步骤证据不足”。

### 定量段落约束
- 单段建议不超过 180 中文字，超长自动拆段。
- 避免 `a=...; b=...; c=...` 连续堆叠，改为短句或列表。
- 每条关键结论应附证据来源；缺失来源时，结论降级为“待验证”。
