...
### 目标
创建一个统一的src/api客户端来简化所有外部服务调用，提高代码可维护性和一致性。

### 主要改进

1. **统一接口**
   - 封装所有OpenAI兼容的src/api调用
   - 提供一致的错误处理和重试机制
   - 标准化的参数处理
...
```python
from src/api.unified_client import get_api_client
...
### 优势
1. **简化调用**: 所有src/api调用使用统一接口
2. **错误处理**: 内置重试和错误恢复机制
3. **类型安全**: 完整的类型提示支持
4. **易于维护**: 集中管理所有src/api交互逻辑
5. **向前兼容**: 不影响现有功能，逐步迁移

### 迁移计划
1. 保留现有src/api接口不变
2. 逐步将内部调用迁移到统一客户端
3. 最终替换分散的src/api调用代码