...
def visualization_writer(
    ...
    base = name or "visual"
    output_dir = ensure_dir(Path(data_sessions_active_dir) / "artifacts" / plan_id / "visualizations" / style)
    registry = registry or ArtifactRegistry(Path(data_sessions_active_dir))
    ...
    log_dir = ensure_dir(Path(data_sessions_active_dir) / "outputs_logs" / "visualizations")
