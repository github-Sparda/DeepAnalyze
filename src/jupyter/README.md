...
The DeepAnalyze Jupyter Frontend is a Jupyter Notebook interface for the DeepAnalyze data science agent. It provides an interactive environment that connects to an OpenAI-compatible LLM service, enabling data preparation, docs/analysis, modeling, visualization, and report generation.
...
For more details, see [uv installation documentation](https://docs/guides.astral.sh/uv/getting-started/installation/) and [Node.js installation documentation](https://nodejs.org/en/download/package-manager).
...
You can use the `uv add` command to add new dependencies, for data/examples:
```bash
uv add xgboost
```
...
After running `uv run CLI.py`, a `data/sessions/active` directory will be created under the `jupyter` directory as the Jupyter working directory, and `deep_analyze.ipynb` will be created as the docs/analysis file. You can upload the required datasets in Jupyter Lab (or directly copy them to the `data/sessions/active` directory).
...
>>> Perform regression docs/analysis on this data and explain the results
>>> Create a predictive model and evaluate its performance
```

All docs/analysis tasks will be executed in `deep_analyze.ipynb`, and you can view the results and export reports in Jupyter Lab.
