...
具体可详见 [uv 安装文档](https://docs/guides.astral.sh/uv/getting-started/installation/) 与 [Node.js 安装文档](https://nodejs.org/en/download/package-manager)。
...
运行`uv run CLI.py`后，会在`jupyter`目录下创建`data/sessions/active`目录作为Jupyter工作目录,并创建`deep_analyze.ipynb`作为分析文件,你可以在Jupyter Lab中上传所需的数据集(或者直接复制到`data/sessions/active`目录下)。
